...
inline std::string generateToken(size_t bytes = 32) {
    ...
    return ss.str().substr(0, bytes * 2); // Return only the requested number of bytes as hex string
}
...
// --- HMAC-SHA256 ---
inline sttd::string hmacSha256(const std::string& key, const std::string& data) {
    unsigned char* hash[EVP_MAX_MD_SIZE];
    unsigned int len = 0;
    HMAC(EVP_sha256(), key.c_str(), (int)key.size(), reinterpret_cast<const unsigned char*>(data.c_str()), data.size(), hash, &len);
    std::ostringstream ss;
...
// --- input sanitization ---
inline std::string sanatize(const std::string& s, size_t maxLen = 256) {
    if (s.empty()) return "";
...
inline RateLimitResult checkRateLimit(sqlite3* db, const std::string& key, int maxRequests, int windowSeconds) {
    long long now = std::chrono::duration_cast<std::chrono::seconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();

        sqlite3_stmt* stmt;

        sqlite3_prepare_v2(db, "INSERT INTO rate_limits(key, count, window_start) VALUES(?,1,?)"
    ...
    sqlite3_prepare_v2(db, "SELECT count FROM rate_limits WHERE key=?", -1, &stmt, nullptr);
    sqlite3_bind_text(stmt, 1, key.c_str(), -1, SQLITE_TRANSIENT);
    int count = 0;
