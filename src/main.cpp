...
void seedDemoAccount() {
    ...
    // Insert user
    sqlite3_exec(db, R"(
        INSERT OR IGNORE INTO users (github_id, username, display_name, avatar_url, bio, location, role, github_url, public, theme, webhook_secret, created_at)
        VALUES ('demo-hackclub-0000', 'hackclub-demo', 'Hack Club',
                ...
                1, 'dark', 'demo-secret', strftime('%s','now'));
    )", nullptr, nullptr, &err);
    ...
    // Clear and re-insert activity
    sqlite3_exec(db, "DELETE FROM activity_feed WHERE user_id = (SELECT user_id FROM users WHERE username = 'hackclub-demo');", nullptr, nullptr, &err);
    if (err) { sqlite3_free(err); err = nullptr; }
    ...
    const char* acts[] = {
        ...
        "INSERT INTO activity_feed (user_id, repo, message, language, commit_sha, pushed_at) SELECT user_id, 'hackclub/putting-in-work', 'chore: update deps, fix security warnings', 'Python', 'c3d4e5f', strftime('%s','now') - 259200 FROM users WHERE username = 'hackclub-demo';",
        nullptr
    };
    for (int i = 0; acts[i]; i++) {
        sqlite3_exec(db, acts[i], nullptr, nullptr, &err);
        if (err) { sqlite3_free(err); err = nullptr; }
    }

    std::cout << "[Demo] hackclub-demo account seeded\n";
}
